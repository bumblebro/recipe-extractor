...
function WhiskingAnimation({ isPaused }: { isPaused: boolean }) {
  ...
  useFrame((state) => {
    if (!isPaused && whiskRef.current && bowlRef.current) {
      ...
      // Realistic whisking motion - figure-8 pattern
  ...
  return (
    <group>
      ...
      <ParticleSystem count={100} color="#ffffff" />
      {bubbles.map((pos, i) => (
...
function CuttingAnimation({ isPaused }: { isPaused: boolean }) {
  ...
  useFrame((state) => {
    if (!isPaused && knifeRef.current && cuttingBoardRef.current) {
      ...
      // Realistic chopping motion - up and down with slight forward movement
      ...
      // Cutting board movement - slight bounce on impact
      cuttingBoardRef.current.position.y =
        -0.5 + Math.abs(Math.sin(cuttingMotion)) * 0.02;

      // Enhanced particle generation on impact
      if (Math.abs(Math.sin(cuttingMotion)) > 0.95) {
        ...
        // Add cutting particles with realistic spread
        for (let i = 0; i < 8; i++) {
          const angle = Math.random() * Math.PI * 2;
  ...
  return (
    <group>
      ...
      <ParticleSystem count={50} color="#8B4513" />
      {particles.map((pos, i) => (
        <mesh key={i} position={pos}>
          <sphereGeometry args={[0.02, 8, 8]} />
          <meshStandardMaterial
  ...
  return (
    <div className="w-full h-full">
      ...
      >
        ...
        <Physics gravity={[0, -9.8, 0]}>{getAnimationComponent()}</Physics>
